...
public class ContentProviderTimeoutCache
    ...
    private int putValues(String acronym,
        ...
        for (int i = 0; i < longForms.size(); i++) {
            ...
            toPut.put(AcronymEntry.COLUMN_ACRONYM, acronym);
            toPut.put(AcronymEntry.COLUMN_EXPIRATION_TIME, expirationTime);
            toPut.put(AcronymEntry.COLUMN_FREQUENCY, longForms.get(i).getFreq());
            toPut.put(AcronymEntry.COLUMN_LONG_FORM, longForms.get(i).getLf());
            toPut.put(AcronymEntry.COLUMN_SINCE, longForms.get(i).getSince());
    ...
    public void removeExpiredAcronyms() {
        ...
        try (Cursor expiredData = mContext.getContentResolver().query(AcronymEntry.CONTENT_URI, new String[]{AcronymEntry.COLUMN_ACRONYM}, SELECTION_EXPIRATION, selectionArgs, null)){
            if(expiredData != null && expiredData.moveToFirst())
            {
                do {
                    String deleteLocation = expiredData.getString(expiredData.getColumnIndex(AcronymEntry.COLUMN_ACRONYM));
                    mContext.getContentResolver().delete(AcronymEntry.CONTENT_URI, AcronymEntry.COLUMN_ACRONYM + " = ?", new String[]{deleteLocation});
